...
const uiManager = new UIManager(
    ...
    async (path) => {
        try {
            ...
            uiManager.addOutputLine(`📂 Opened: ${path}`);
        } catch (e) {
            uiManager.addOutputLine(`❌ Failed to open: ${path}`);
        }
    },
    // onTogglePause
    () => {
        ...
        uiManager.addOutputLine(isPaused ? '⏸ Paused streaming' : '▶ Resumed streaming');
    },
    // onSkipStreaming
    () => {
        if (editorManager.getIsStreaming()) {
            ...
            uiManager.addOutputLine('⏭ Skipped to end');
        }
    }
);
    ...
    switch (data.type) {
        case 'step':
            if (data.file_op) {
                ...
                uiManager.addOutputLine(`${action === 'write' ? '📝' : '📖'} ${action}: ${path}`);
                if (action === 'write') {
        ...
        case 'complete':
            ...
            uiManager.addOutputLine(`✅ ${content}`);
            break;
        case 'stopped':
        ...
        case 'file_content':
            ...
            uiManager.addOutputLine(`📂 Opened: ${data.path}`);
            break;
    }
}
